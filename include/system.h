...
/* Инициализация процесса-демона.
   Данную функцию может вызывать приложение, желающее стать демоном.
   ...
   cmd - указатель на строку, которая будет добавляться в начало каждого
   сообщения при работе функции syslog. Необходим для инициализации файла 
   журнала. При cmd == NULL автоматически будет использоваться имя
   программы.
