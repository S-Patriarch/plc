...
struct net_ctx_s;  /* слушатель / клиент  */
struct net_buf_s;  /* буфер сообщений  */
...
struct net_opts_s {
        enum net_family_e family;  /* NET_IPV4 / NET_IPV6 / NET_ANY  */
        enum net_proto_e  proto;   /* NET_TCP  */
...
extern struct net_ctx_s *p_net_create (const struct net_opts_s *opts, 
                                       int do_bind) P_NOEXCEPT;

/* bind + listen для сервера.
   backlog - размер очереди ожидающих соединений.
   Возвращает 0 при успехе, -1 при ошибке (errno).  */
extern int p_net_listen (struct net_ctx_s *ctx, int backlog) P_NOEXCEPT;

/* Принять входящее соединение (блокирующий или нет - зависит от флага).
   Возвращает нового net_ctx_s * или NULL + errno.  */
extern struct net_ctx_s *p_net_accept (struct net_ctx_s *ctx) P_NOEXCEPT;

/* Подключиться к серверу (клиент).
   Возвращает 0 при успехе, -1 при ошибке (errno).  */
extern int p_net_connect (struct net_ctx_s *ctx) P_NOEXCEPT;

/* Закрыть и освободить net_ctx_s (безопасно для NULL).  */
extern void p_net_free (struct net_ctx_s *ctx) P_NOEXCEPT;
...
extern struct net_buf_s *p_net_recv (struct net_ctx_s *ctx) P_NOEXCEPT;
...
extern int p_net_send (struct net_ctx_s *ctx, const void *data, size_t len) 
        P_NOEXCEPT;

/* Удобная обертка: отправить строку (без завершающего нуля).
   Возвращает то же, что p_net_send.  */
extern int p_net_send_str (struct net_ctx_s *ctx, const char *str) P_NOEXCEPT;
...
extern const void *p_net_buf_data (const struct net_buf_s *buf) P_NOEXCEPT;

/* Размер данных в буфере.  */
extern size_t p_net_buf_size (const struct net_buf_s *buf) P_NOEXCEPT;

/* Освободить буфер (безопасно для NULL).  */
extern void p_net_buf_free (struct net_buf_s *buf) P_NOEXCEPT;
...
extern int p_net_set_timeout (struct net_ctx_s *ctx, int recv_ms, int send_ms) 
        P_NOEXCEPT;

/* Установить TTL (Time-To-Live) для сокета.
   Возвращает 0 при успехе, -1 при ошибке.  */
extern int p_net_set_ttl (struct net_ctx_s *ctx, int ttl) P_NOEXCEPT;
...
extern int p_net_get_fd (const struct net_ctx_s *ctx) P_NOEXCEPT;
