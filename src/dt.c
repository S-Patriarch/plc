...
int 
p_gettime(char *s, size_t size) P_NOEXCEPT
{
        time_t now;
...
int 
p_date_convert(const char *s_in, size_t size_s_in, char *s_out, 
               size_t size_s_out, enum dcf dcf) P_NOEXCEPT
{
        if (s_in == NULL || *s_in == '\0')
                return (P_ERROR);
...
void 
p_delay_ms(unsigned int ms) P_NOEXCEPT
{
        struct timespec ts;
